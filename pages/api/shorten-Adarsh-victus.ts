...
export default apiHandler(async (req, res) => {
    ...
    if (cookieHeader) {
        ...
        if (sessionId) {
            ...
            if (session) {
                ...
                // FREE PLAN LIMIT: Max 30 Links
                if (user?.plan === 'free') {
                    const linkCount = await db.link.count({ where: { userId } })
                    if (linkCount >= 30) {
                        throw new AppError('Free plan limit reached (30 links). Upgrade to Premium for unlimited links.', 402)
                    }
                }
            }
        }
    ...
    if (note) {
        if (!userId) {
            // If not logged in, cannot use premium features
            // We can either ignore the note or throw an error. 
            // Throwing error is better feedback.
